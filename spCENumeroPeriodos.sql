...
--//WITH ENCRYPTION
AS BEGIN  
  DECLARE @DurPeriodos int, @PeriodoInicio int, @Contador int  
  ...
  --DELETE FROM @Tabla WHERE Periodo IN (SELECT ISNULL(PeriodoAsig,0) FROM CE a JOIN MovTipo b ON a.Mov = b.Mov AND Modulo = 'CE' AND Clave IN ('CE.CP','CE.ACL') WHERE Empresa = @Empresa AND Sucursal = @Sucursal AND CicloEscolar = @CicloEscolar AND Materia = @Materia AND a.Grupo = @Grupo AND Estatus = 'CONCLUIDO')
